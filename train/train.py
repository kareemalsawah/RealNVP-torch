...
def train_epoch(model, optimizer, train_loader, test_pass=False):
...
def test(model, test_loader, test_pass=False):
    device = next(model.parameters()).device
...
def train(model, optimizer, n_epochs, train_loader, test_loader, plot=False, figname=None, test_pass=False):
    train_losses = []
    test_losses = []

    for e in range(1, n_epochs+1):
        test_loss_epoch = test(model,test_loader,test_pass)
        if e > 1:
            test_losses.append(test_loss_epoch)

        print("Epoch {}, Test Loss: {}".format(e, test_loss_epoch))

        train_loss_epoch = train_epoch(model,optimizer,train_loader,test_pass)
        if e > 1:
...
def generate_samples(model,num_samples=32,floor=True,num_to_plot=0,samples_per_row=5,figsize=(15,8),figname=None):
    device = next(model.parameters()).device
